...
export default function Aside() {
  ...
  return (
    ...
    >
            ...
            if (minimize) {
              return (
                ...
                >
                  ...
                  <Icon size={20} className={isActive ? 'text-[#707dff]' : 'text-[#5a6382]'} />
                </Link>
              )
            }
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-center size-[20px] ml-[18px] shrink-0">
                  <Icon size={18} className={isActive ? 'text-[#707dff]' : 'text-[#5a6382]'} />
                </div>
                <div className="flex-1 min-w-px px-[20px]">
                  <span
                    className={`block text-[13.5px] tracking-[0.108px] whitespace-nowrap ${
                      isActive
      ...
      <div className="border-t border-[#eceef8] flex flex-col gap-[4px] h-[110px] items-start justify-center px-[8px] py-[10px] shrink-0 w-full">
        {minimize ? (
          <>
            ...
            <div className="flex items-center justify-center h-[48px] px-[10.5px] py-[7px] rounded-[10px] hover:bg-[rgba(112,125,255,0.05)] transition-colors w-full">
              <div
                ...
                style={{ backgroundImage: 'linear-gradient(135deg, #707dff 0%, #5062f5 60%, #3a52ef 100%)' }}
              >
                <span className="text-white text-[12.5px] font-bold tracking-[0.5px]">{initials}</span>
              </div>
            </div>
          </>
        ) : (
          <>
            <div className="flex gap-[4px] items-center w-full">
              ...
              <div className="flex-1 flex gap-[4px] items-center min-w-px pr-[10px]">
                <span className="flex-1 text-[13.5px] font-medium text-[#5a6382]">Notifications</span>
                <span className="bg-[#fe6f6f] text-white text-[10.5px] font-bold leading-[15.75px] px-[6px] rounded-[10px] h-[19px] flex items-center">3</span>
              </div>
            </div>
            ...
            <div className="flex gap-[4px] items-center w-full">
              ...
              <div className="flex flex-col gap-px w-[158px]">
                <span className="text-[13px] font-bold text-[#12143a] truncate">{session?.user?.name || 'User'}</span>
                <span className="text-[11px] font-medium text-[#9ea8c6] truncate">{session?.user?.role || 'USER'}</span>
              </div>
            </div>
          </>
        )}
